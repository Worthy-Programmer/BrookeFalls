<script context="module">
  import dayjs from 'dayjs';

  export async function load({ fetch, page: { host } }) {
    const posts = (
      await (await fetch(`https://${host}/.netlify/functions/posts`)).json()
    ).map((post) => {
      post.slug = getSlug(post.meta.path);
      return post;
    });
    posts.sort(sortPostsByDate);
    return {
      props: { posts },
    };
  }

  function sortPostsByDate(a, b) {
    dayjs(a.meta.date, 'MMM D, YYYY') - dayjs(b.meta.date, 'MMM D, YYYY');
  }
  function getSlug(path) {
    return path.slice(0, -3).substring(path.lastIndexOf('/') + 1);
  }
</script>

<script>
  import { base } from '$app/paths';
  import Post from '$lib/PostSide.svelte';
  import PostSide from '$lib/PostSide.svelte';
  import brookePretty from '../images/brookePretty.jpg';
  import brookeMic from '../images/brookeMic.jpg';
  import brookePolkadot from '../images/brookePolkadot.jpg';

  export let posts;

  let sidebarItems = [
    { title: 'Videographer', faClass: 'fas fa-video fa' },
    { title: 'Photographer', faClass: 'fas fa-camera-retro fa' },
    { title: 'Vocalists', faClass: 'fas fa-microphone-alt fa' },
    { title: 'Musicians', faClass: 'fas fa-music fa' },
  ];

  let sidebarItemLinks = [
    { title: 'Portfolio', to: '/' },
    { title: 'About', to: '/about' },
    { title: 'Play', to: '/play' },
  ];

  let socialMediaIcons = [
    { title: 'Twitter', faClass: 'fab fa-twitter fa' },
    { title: 'Facebook', faClass: 'fab fa-facebook fa' },
    { title: 'Instagram', faClass: 'fab fa-instagram fa' },
  ];
</script>

<svelte:head>
  <title>Home</title>
</svelte:head>

<main class="flex flex-col h-screen">
  <div class="flex flex-1 overflow-hidden">
    <div class="flex bg-gray-100 xl:w-64 xs:hidden md:block">
      <div class="mx-6 my-6">
        <ul class="flex flex-col font-headline">
          <li class="text-gray-500 py-4 tracking-wider">I'm hiring!</li>

          {#each sidebarItems as item}
            <li
              class="tracking-wider px-3 hover:bg-gray-300 cursor-pointer rounded-lg pl-4 h-8 flex items-center"
            >
              <span><i class={item.faClass} /> {item.title}</span>
            </li>
          {/each}
        </ul>
        <ul class="flex flex-col font-headline">
          <li class="text-gray-500 py-4 tracking-wider">Links!</li>
          {#each sidebarItemLinks as link}
            <li
              class="tracking-wider px-3 hover:bg-gray-300 cursor-pointer rounded-lg pl-4 h-8 flex items-center"
            >
              <a href={link.to}>
                <span class="text-gray-900">
                  {link.title}
                </span></a
              >
            </li>
          {/each}
        </ul>
        <div class="bg-white flex flex-col h-64 items-center justify-center">
          <ul class="font-headline">
            {#each socialMediaIcons as socialIcon}
              <li
                class="tracking-wider px-3 hover:bg-gray-300 cursor-pointer rounded-lg pl-4 h-8 flex items-center py-9"
              >
                <span><i class={socialIcon.faClass} /> {socialIcon.title}</span>
              </li>
            {/each}
          </ul>
        </div>
      </div>
    </div>
    <div class="flex flex-1 flex-col items-center overflow-y-scroll">
      <div class="flex flex-1">
        <div>
          <div class="bg-gray-100">
            <!-- featured -->
            <div class="max-w-5xl mx-auto pt-10 pb-10">
              <ul class="flex flex-wrap -mx-2 overflow-hidden">
                <li
                  class="my-2 px-2 w-full overflow-hidden md:w-1/3 lg:w-1/3 xl:w-1/3"
                >
                  <a href="">
                    <div
                      class="mx-2 h-96 flex items-center justify-center bg-gray-300 bg-cover bg-center relative rounded overflow-hidden"
                      style=" background-image:url({brookePolkadot})"
                    >
                      <div
                        class="absolute w-full h-full bg-black z-10 opacity-50"
                      />
                      <div class="relative z-20 text-center p-5">
                        <span
                          class="inline-block text-white uppercase text-xs tracking-wide"
                          >Showcases</span
                        >
                        <h2
                          class="text-white font-semibold font-serif text-xl my-5"
                        >
                          Come visit us Thursday nights!
                        </h2>
                        <span class="inline-block text-xs text-white font-sans"
                          >Comfort and views</span
                        >
                      </div>
                    </div>
                  </a>
                </li>
                <li
                  class="my-2 px-2 w-full overflow-hidden md:w-1/3 lg:w-1/3 xl:w-1/3"
                >
                  <a href="">
                    <div
                      class="mx-2 h-96  flex items-center justify-center bg-gray-300 bg-cover bg-center relative rounded overflow-hidden"
                      style="background-image:url({brookePretty})"
                    >
                      <div
                        class="absolute w-full h-full bg-black z-10 opacity-50"
                      />
                      <div class="relative z-20 text-center p-5">
                        <span
                          class="inline-block text-white uppercase text-xs tracking-wide"
                          >NYC</span
                        >
                        <h2
                          class="text-white font-semibold font-serif text-xl my-5"
                        >
                          Pretty Woman
                        </h2>
                        <span class="inline-block text-xs text-white font-sans"
                          >The Playhouse in NYC</span
                        >
                      </div>
                    </div>
                  </a>
                </li>
                <li
                  class="my-2 px-2 w-full overflow-hidden md:w-1/3 lg:w-1/3 xl:w-1/3"
                >
                  <a href="">
                    <div
                      class="mx-2 flex h-96  items-center justify-center bg-gray-300 bg-cover bg-center relative rounded overflow-hidden"
                      style=" background-image:url({brookeMic})"
                    >
                      <div
                        class="absolute w-full h-full bg-black z-10 opacity-50"
                      />
                      <div class="relative z-20 text-center p-5">
                        <span
                          class="inline-block text-white uppercase text-xs tracking-wide"
                          >Celine Dion</span
                        >
                        <h2
                          class="text-white font-semibold font-serif text-xl my-5"
                        >
                          Singing my favorite artist
                        </h2>
                        <span class="inline-block text-xs text-white font-sans"
                          >My heart will go on</span
                        >
                      </div>
                    </div>
                  </a>
                </li>
              </ul>
            </div>
          </div>
          <div class="max-w-5xl mx-auto pb-10 pt-10">
            <div class="flex flex-wrap overflow-hidden">
              <div class="w-full overflow-hidden md:w-4/6 lg:w-4/6 xl:w-4/6">
                <div class="mr-2 md:mr-4 ml-2">
                  <div class="pb-10">
                    <div>
                      <img
                        class="article-image"
                        src={posts[0].meta.url}
                        alt=""
                      />
                      <h2
                        class="text-gray-900 font-serif text-3xl my-5 font-thin"
                      >
                        <a href="">How To Make More Travel By Doing Less</a>
                      </h2>
                      <span class="text-xs text-gray-800 font-thin block mb-5"
                        >By Page. December 02, 2019</span
                      >
                      <p class="article-body">
                        Far far away, behind the word mountains, far from the
                        countries Vokalia and Consonantia, there live the blind
                        texts. Separated they live in Bookmarksgrove right at
                        the coast...
                      </p>
                      <a href={`${base}/${posts[0].slug}`} class="readmore"
                        >Read More...</a
                      >
                    </div>
                  </div>
                  <div class="article-row">
                    {#each posts as post}
                      <Post {post} {base} />
                    {/each}
                  </div>
                </div>
              </div>

              <div class="w-full overflow-hidden md:w-2/6 lg:w-2/6 xl:w-2/6">
                <!-- sidebar -->
                <div class="ml-2 md:ml-4 mr-2">
                  <div class="mt-20 sm:mt-0 text-center">
                    <img
                      class="w-64 h-64 rounded-full mx-auto"
                      src="https://media.istockphoto.com/photos/studio-microphone-and-pop-shield-on-mic-in-the-empty-recording-studio-picture-id1279654034?b=1&k=20&m=1279654034&s=170667a&w=0&h=jebzMhp_tlJi-3fLn3Ig8cYWG_JaF-vjt4SWLAI9o9Q="
                      alt=""
                    />
                    <h2 class="font-light text-xl my-5">We are</h2>
                    <p class="article-body">
                      Hi! Welcome to Brooke Falls, your guide to the most
                      beautiful and unique island in Maldives.
                    </p>
                  </div>
                  <div class="mt-20">
                    <h2 class="font-light text-xl mb-5 text-center">
                      Search the blog
                    </h2>
                    <div class="relative border rounded-sm overflow-hidden">
                      <form action="">
                        <input
                          class="w-full relative p-5 font-light text-gray-900 border-0"
                          type="text"
                          name="s"
                          id=""
                          placeholder="Search..."
                        />
                        <button
                          type="submit"
                          class="bg-transparent border-0 absolute right-0 px-5 py-5 top-2"
                        >
                          <span class="block w-5">
                            <svg
                              class="fill-current"
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 512 512"
                              ><path
                                d="M495 466.2L377.2 348.4c29.2-35.6 46.8-81.2 46.8-130.9C424 103.5 331.5 11 217.5 11 103.4 11 11 103.5 11 217.5S103.4 424 217.5 424c49.7 0 95.2-17.5 130.8-46.7L466.1 495c8 8 20.9 8 28.9 0 8-7.9 8-20.9 0-28.8zm-277.5-83.3C126.2 382.9 52 308.7 52 217.5S126.2 52 217.5 52C308.7 52 383 126.3 383 217.5s-74.3 165.4-165.5 165.4z"
                              /></svg
                            >
                          </span>
                        </button>
                      </form>
                    </div>
                  </div>
                  <div class="mt-10 bg-gray-100 rounded-sm p-5">
                    <div class="pb-6">
                      <div class="w-10 mx-auto mt-6 text-gray-900">
                        <svg
                          class="fill-current"
                          viewBox="-1 0 512 512"
                          xmlns="http://www.w3.org/2000/svg"
                          ><path
                            d="M505.668 246.465c-.89-.906-54.297-54.309-55.668-55.68V55c0-30.328-24.672-55-55-55H115C84.672 0 60 24.672 60 55v135.785C.379 250.406 0 248.301 0 257v210c0 24.813 20.188 45 45 45h420c24.813 0 45-20.188 45-45V257c0-3.855-1.54-7.71-4.332-10.535zm-35.992 6.426L450 262.73v-29.516zM115 30h280c13.785 0 25 11.215 25 25v222.73l-120 60V197c0-8.285-6.715-15-15-15H135c-8.285 0-15 6.715-15 15v95.73l-30-15V55c0-13.785 11.215-25 25-25zm155 257.973l-66.68-44.453a15.004 15.004 0 00-15.027-.938L150 261.73V212h120zm-120 7.297l43.922-21.961L270 324.027v28.703l-15 7.5-105-52.5zm-90-32.54l-19.676-9.84L60 233.216zM465 482H45c-8.27 0-15-6.73-15-15V281.27l218.293 109.148a15.008 15.008 0 0013.414 0L480 281.27V467c0 8.27-6.73 15-15 15zm0 0"
                          /><path
                            d="M195 91h120c8.285 0 15-6.715 15-15s-6.715-15-15-15H195c-8.285 0-15 6.715-15 15s6.715 15 15 15zm0 0M135 151h240c8.285 0 15-6.715 15-15s-6.715-15-15-15H135c-8.285 0-15 6.715-15 15s6.715 15 15 15zm0 0M375 181h-30c-8.285 0-15 6.715-15 15s6.715 15 15 15h30c8.285 0 15-6.715 15-15s-6.715-15-15-15zm0 0M375 241h-30c-8.285 0-15 6.715-15 15s6.715 15 15 15h30c8.285 0 15-6.715 15-15s-6.715-15-15-15zm0 0"
                          /></svg
                        >
                      </div>
                      <h2
                        class="font-light text-xl mb-2 text-center pt-5 text-gray-900"
                      >
                        Subscribe to our newsletter
                      </h2>
                      <span
                        class="block text-center article-body text-xs italic"
                        >Get the news right in your inbox!</span
                      >
                      <form action="">
                        <div
                          class="bg-white border border-gray-400 mt-5 rounded-sm overflow-hidden"
                        >
                          <input
                            class="bg-transparent w-full p-3"
                            type="text"
                            name="name"
                            id=""
                            placeholder="First Name"
                          />
                        </div>
                        <div
                          class="bg-white border border-gray-400 mt-3 rounded-sm overflow-hidden"
                        >
                          <input
                            class="bg-transparent w-full p-3"
                            type="email"
                            name="email"
                            id=""
                            placeholder="Email Address"
                          />
                        </div>
                        <div class="mt-3 article-body text-xs italic">
                          <span class="inline-block pr-1">
                            <input type="checkbox" name="" id="privacy-check" />
                          </span>
                          <label for="privacy-check"
                            >By checking this you agree to our <a
                              href=""
                              class="text-orange-500">Privacy Policy</a
                            >.</label
                          >
                        </div>
                        <button
                          type="submit"
                          class="w-full rounded-sm bg-gray-900 text-white tracking-widest text-sm uppercase font-medium py-3 mt-5"
                          >Subscribe</button
                        >
                      </form>
                    </div>
                  </div>
                  <div class="mt-10">
                    <h2
                      class="font-light text-xl mb-5 text-gray-900 text-center"
                    >
                      Categories
                    </h2>
                    <ul>
                      <li class="flex">
                        <a
                          href=""
                          class="text-gray-900 font-thin font-serif text-lg py-2 block flex-1"
                          >Travel Tips</a
                        ><span class="text-gray-700 text-lg font-thin p-2"
                          >2</span
                        >
                      </li>
                      <li class="flex">
                        <a
                          href=""
                          class="text-gray-900 font-thin font-serif text-lg py-2 block flex-1"
                          >Shark Life</a
                        ><span class="text-gray-700 text-lg font-thin p-2"
                          >10</span
                        >
                      </li>
                      <li class="flex">
                        <a
                          href=""
                          class="text-gray-900 font-thin font-serif text-lg py-2 block flex-1"
                          >Inspiration</a
                        ><span class="text-gray-700 text-lg font-thin p-2"
                          >7</span
                        >
                      </li>
                      <li class="flex">
                        <a
                          href=""
                          class="text-gray-900 font-thin font-serif text-lg py-2 block flex-1"
                          >Travel Gear</a
                        ><span class="text-gray-700 text-lg font-thin p-2"
                          >4</span
                        >
                      </li>
                    </ul>
                  </div>
                  <div class="mt-10">
                    <h2
                      class="font-light text-xl mb-5 text-gray-900 text-center"
                    >
                      Recent Posts
                    </h2>
                    <ul>
                      {#each posts.slice(0, 3) as post}
                        <PostSide {post} {base} />
                      {/each}
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div class="text-center">
              <button
                class="rounded-sm bg-gray-900 text-white tracking-widest text-sm uppercase font-medium py-3 mt-5 px-5"
                >Load more articles</button
              >
            </div>
          </div>
          <footer class="bg-gray-100 text-center sm:text-left">
            <div class="max-w-5xl mx-auto pb-10 pt-10 text-gray-900">
              <ul>
                <li class="inline-block">
                  <a class="block font-semibold px-3" href="./index.html"
                    >Home</a
                  >
                </li>
                <li class="inline-block">
                  <a class="block font-semibold px-3" href="./categories.html"
                    >Categories</a
                  >
                </li>
                <li class="inline-block">
                  <a class="block font-semibold px-3" href="./about.html"
                    >About</a
                  >
                </li>
              </ul>
              <div class="sm:flex px-3">
                <div class="w-full sm:w-1/2">
                  <h4 class="font-bold text-2xl font-serif pt-10">
                    Brooke Falls
                  </h4>
                  <span class="block pt-1 text-xs tracking-wider font-light"
                    >&copy;2021 Brooke Falls. All rights reserved.</span
                  >
                </div>
                <div
                  class="w-full sm:w-1/2 text-center md:text-right mt-10 sm:mt-0"
                >
                  <a href="" title="Facebook" class="inline-block w-6 mr-2">
                    <svg
                      class="fill-current"
                      viewBox="-110 1 511 512"
                      xmlns="http://www.w3.org/2000/svg"
                      ><path
                        d="M180 512H98.008c-13.695 0-24.836-11.14-24.836-24.836V302.227H25.336C11.64 302.227.5 291.082.5 277.39v-79.246c0-13.696 11.14-24.836 24.836-24.836h47.836v-39.684c0-39.348 12.355-72.824 35.726-96.805C132.375 12.73 165.184 0 203.778 0l62.53.102c13.672.023 24.794 11.164 24.794 24.835v73.579c0 13.695-11.137 24.836-24.829 24.836l-42.101.015c-12.84 0-16.11 2.574-16.809 3.363-1.152 1.31-2.523 5.008-2.523 15.223v31.352h58.27c4.386 0 8.636 1.082 12.288 3.12 7.88 4.403 12.778 12.727 12.778 21.723l-.031 79.247c0 13.687-11.141 24.828-24.836 24.828h-58.47v184.941C204.84 500.86 193.696 512 180 512zm-76.813-30.016h71.633V288.79c0-9.144 7.442-16.582 16.582-16.582h66.727l.027-68.883h-66.758c-9.14 0-16.578-7.437-16.578-16.582v-44.789c0-11.726 1.192-25.062 10.043-35.086 10.696-12.117 27.551-13.515 39.301-13.515l36.922-.016V30.109l-57.332-.093c-62.024 0-100.566 39.703-100.566 103.609v53.117c0 9.14-7.438 16.582-16.579 16.582H30.516v68.883h56.093c9.141 0 16.579 7.438 16.579 16.582zM266.25 30.117h.004zm0 0"
                      /></svg
                    >
                  </a>
                  <a href="" title="Twitter" class="inline-block w-6 mr-2">
                    <svg
                      class="fill-current"
                      viewBox="0 0 512.001 512.001"
                      xmlns="http://www.w3.org/2000/svg"
                      ><path
                        d="M373.406 0H138.594C62.172 0 0 62.172 0 138.594V373.41C0 449.828 62.172 512 138.594 512H373.41C449.828 512 512 449.828 512 373.41V138.594C512 62.172 449.828 0 373.406 0zm108.578 373.41c0 59.867-48.707 108.574-108.578 108.574H138.594c-59.871 0-108.578-48.707-108.578-108.574V138.594c0-59.871 48.707-108.578 108.578-108.578H373.41c59.867 0 108.574 48.707 108.574 108.578zm0 0"
                      /><path
                        d="M256 116.004c-77.195 0-139.996 62.8-139.996 139.996S178.804 395.996 256 395.996 395.996 333.196 395.996 256 333.196 116.004 256 116.004zm0 249.976c-60.64 0-109.98-49.335-109.98-109.98 0-60.64 49.34-109.98 109.98-109.98 60.645 0 109.98 49.34 109.98 109.98 0 60.645-49.335 109.98-109.98 109.98zm0 0M399.344 66.285c-22.813 0-41.367 18.559-41.367 41.367 0 22.813 18.554 41.371 41.367 41.371s41.37-18.558 41.37-41.37-18.558-41.368-41.37-41.368zm0 52.719c-6.258 0-11.352-5.094-11.352-11.352 0-6.261 5.094-11.351 11.352-11.351 6.261 0 11.355 5.09 11.355 11.351 0 6.258-5.094 11.352-11.355 11.352zm0 0"
                      /></svg
                    >
                  </a>
                  <a href="" title="Instagram" class="inline-block w-6">
                    <svg
                      class="fill-current"
                      viewBox="0 -45 512 512"
                      xmlns="http://www.w3.org/2000/svg"
                      ><path
                        d="M194.816 422.71c-85.453 0-152.992-14.929-185.304-40.952l-.63-.508-.57-.57C.638 373-1.877 362.926 1.418 353.043l.3-.82c4-10.004 13.802-16.739 24.423-16.801 21.859-.379 40.984-2.985 58.34-8.043-27.684-12.875-46.915-35.168-58.356-67.434-3.848-10.062-.527-21.218 8.34-27.87 2.187-1.645 4.66-2.887 7.277-3.72-15.383-17.757-26.746-37.964-33.11-59.335l-.198-.665-.137-.68c-2.16-10.808 2.672-21.921 11.527-26.706 3.715-2.133 7.75-3.239 11.801-3.332-4.367-9.407-7.543-19.063-9.426-28.778-5.226-26.921-.914-53.91 12.813-80.214l3.175-6.352a17.759 17.759 0 0113.004-9.563 17.76 17.76 0 0115.356 4.958l5.785 5.792c45.703 47.915 86.64 70.649 157.418 86.204 3.16-27.168 14.906-52.422 33.855-72.297C286.156 13.738 316.27.469 348.398.016h.211c23.442 0 52.52 13.382 70.106 22.82 15.086-4.938 33.262-12.582 52.12-20.664 8.825-4.14 19.704-2.246 26.642 4.691 6.8 6.801 8.687 16.39 5.078 25.711a126.223 126.223 0 01-4.657 11.227c2.582 1.183 4.946 2.789 6.942 4.785 6.035 6.035 8.55 15.48 6.406 24.066l-.23.817c-7.227 23.289-21.11 42.258-39.47 54.164C468.48 290.918 345.52 422.71 194.817 422.71zM38.305 365.036c30.449 17.227 88.476 27.649 156.511 27.649 65.41 0 127.137-28.082 173.805-79.075 47.05-51.41 72.961-119.68 72.961-192.234v-.816c0-6.57 3.617-12.567 9.438-15.653C462.828 98.652 472.39 88 478.61 74.38c-6.415 1.012-12.934-1.563-16.93-6.93a17.621 17.621 0 01-.13-20.957 97.453 97.453 0 006.692-10.59c-16.285 6.84-31.75 12.973-45.176 17.047-4.878 1.477-10.316.899-14.773-1.59-23.902-13.316-46.164-21.277-59.586-21.316-49.527.758-89.797 43.176-89.797 94.605 0 5.317-2.36 10.301-6.465 13.68a17.646 17.646 0 01-14.672 3.695c-81.609-16.078-129.968-40.187-180.257-90.722-7.207 17.27-9.176 34.664-5.844 51.84 3.379 17.398 12.367 34.832 25.996 50.414 5.18 5.914 5.867 14.375 1.71 21.05-4.14 6.653-12.01 9.762-19.577 7.735-5.914-1.586-11.352-3.668-16.508-6.344 10.504 22.816 28.57 43.918 51.281 59.48 6.582 4.512 9.332 12.922 6.692 20.454-2.645 7.543-10.032 12.398-18.016 11.804-8.7-.644-16.406-2.296-23.5-5.082 12.734 25.934 33.082 40.203 64.43 45.657a17.72 17.72 0 0114.465 14.668 17.714 17.714 0 01-9.2 18.43c-26.718 14.163-55.922 21.765-91.14 23.628zm0 0"
                      /></svg
                    >
                  </a>
                </div>
              </div>
            </div>
          </footer>
        </div>
      </div>
    </div>
  </div>
  <div class="flex">Footer</div>
</main>
