...
>
  <div class="p-3 w-8 flex-shrink-0 animate-pulse">▶️</div>
  <div class="p-3 w-8 flex-shrink-0">🎧</div>
  <div class="p-3 w-full">{song.name}</div>
  <div class="p-3 w-full xs:hidden sm:block">
    {song.artists.map((artist) => {
      return artist.name;
    })}
  </div>
  <div class="p-3 w-full">{song.album.name}</div>
  <div class="p-3 w-12 xs:hidden sm:block flex-shrink-0 text-right">
